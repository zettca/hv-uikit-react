...
<Callout type="info">
  You can also enable the `autoViewsAndRoutes` option in the `vite.config.ts`
  file and when set it will search for _Views_ at the folder specified by the
  `viewsFolder` parameter (`src/pages` by default) and add them automatically.
  For details on file-based routing, refer to the [plugin
  documentation](../client/packages/app-shell-vite-plugin/README.md). Note that
  _Shared Modules_ other than _Views_ must still be listed in the `modules`
  parameter at the `vite.config.ts` file.
</Callout>
