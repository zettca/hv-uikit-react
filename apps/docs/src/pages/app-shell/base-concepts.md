...
- **Provider**: Module exporting a React Component that provides a React Context to be used by other _Shared Modules_. It's configured in the **App Shell** configuration file in the `providers` array. Detailed in the [State](./state) section.
- **Theme**: Module that exports a UI Kit theme definition. They can be referenced in the **App Shell** configuration file in the `theming.themes` array. They're detailed in the [Theming](./theming) section.
